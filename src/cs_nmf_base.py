...
class CS_NMF:
    ...
    def run_cs(self, pa = 0.25, nIter = 100, ind_type = np.ndarray, ind_size = None, nCuckoos = 50, ind_gen = None,
        ...
        self.toolbox.register("l_ranWalk", l_rw, stepSize, _lambda, pa)
        self.toolbox.register("g_ranWalk", g_rw, stepSize, _lambda,)
        ...
        for g in range(nIter):
            ...
            cuckoo = self.toolbox.g_ranWalk(self.__clone(cuckoo), step)
            ...
            new_cuckoos = map(lambda c: self.toolbox.l_ranWalk(self.__clone(c), cuckoos, self.toolbox.stepFunction()), worst_cuckoos)
